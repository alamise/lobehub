...
/**
 * Spawn an external agent CLI (Amp, Claude Code, Codex, Kimi Code, OpenCode,
 * Pi, or Qoder) and yield its stream as unified `AgentStreamEvent`s. Used by
 * `lh hetero exec` for both standalone terminal runs and (later) sandbox-driven
 * runs that ingest into the server.
 *
