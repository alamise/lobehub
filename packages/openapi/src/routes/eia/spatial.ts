/**
 * 准入判定 - 空间冲突检测（三线一单 / 饮用水水源保护区 / 声环境功能区划）。
 *
 * 三个维度统一采用「共享智能体自主跑完整链路」的方案：
 * 智能体自身挂载 hbai-mcp 工具，自行完成 地址识别 → 高德地理编码 → 空间碰撞检测 →
 * 政策或法条关联 → 准入判定；后端只负责组织输入、调用智能体、容错解析其 JSON 输出。
 *
 * 刻意不做任何直连 LLM 降级：降级会绕开智能体自身的提示词与工具配置，结果不可控。
 * 智能体调用失败时直接抛错，由上层如实回显。
 *
 * 历史：早期这三个维度曾是后端硬编码的两阶段本地流水线
 * （1:1 迁移自 legacy-go-project-monorepo backend/internal/service/eia_*_service.go），
 * 现已全部下沉到智能体侧，相关本地管线代码已移除。
 */
